...
RUN dotnet restore "./CursoDez/CursoDez.API.csproj"
COPY . .
WORKDIR "/src/CursoDez"
RUN dotnet build "./CursoDez.API.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./CursoDez.API.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
